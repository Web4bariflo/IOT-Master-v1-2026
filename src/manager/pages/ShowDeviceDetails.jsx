...
const ShowDeviceDetails = ({ clusterid }) => {
  ...
  // ✅ store full pond object instead of only name
  ...
  const fetchPonds = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${BASEURL}/clusterpond_analytic/${clusterid}/`
      );

      setpondList(response.data.pond);

      // ✅ set first pond as default
      if (response.data.pond.length > 0) {
        setActivePond(response.data.pond[0]);
      }
  ...
  return (
    <div className="flex w-full bg-white rounded-lg shadow-sm">
      ...
      {pondList!=0 ? (
          ...
          {pondList.map((pond) => {
            ...
            return (
              <button
                ...
                ${
                  isActive
                    ? `bg-white font-medium border border-gray-200 border-b-0 relative z-10 -mb-px pb-2`
                    : `border-r border-gray-200 text-gray-500 hover:bg-gray-200 hover:text-gray-700`
                }`}
              >
                {pond.name}
              </button>
        ...
        {/* MODULE RENDERING */}
        {activeModule === "feeding" && activePond && (
          <FeedingModule
        ...
        {activeModule === "energy" && activePond && (
          <EnergyModule
            pondId={activePond.id}
            pondName={activePond.name}
          />
