...
import FarmTab from "../../pages/tabs/FarmTab";
import { POND_TABS } from "../../constants/PondTabs";
import PowerMonTab from "../../pages/tabs/PowerMonTab";
import PowerMonMap from "./PowerMonMap"; 
...
const PondSection = ({ activeTab }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="md:col-span-2 w-full">
  {activeTab === POND_TABS.POWER_MON ? (
    <PowerMonMap />   // 👈 show image/map for Power Monitoring
  ) : (
    <PondMap />       // 👈 default pond map
  )}
</div>
        <div className="w-full">
          <PondSummaryCard />
