...
const Feeding = () => {
  ...
  const isAnyCycleRunning = tasks.some(
    (t) => t.status === "processing" || t.status === "active",
  );
    ...
    switch (row.status) {
      case "pending":
        return {
          ...
          onClick: isLocked ? null : () => handleSubmit(row),
        };
                  ...
                  {tasks.length === 0 ? (
                    ...
                    tasks.map((row, index) => (
                      <tr key={row.taskId} className="hover:bg-gray-50">
                        ...
                        <td>
                          <input
                            className="border rounded px-2 h-7 text-xs w-16"
                        ...
                        <td>
                          <select
                            ...
                            value={row.worker}
                            onChange={(e) =>
                              updateTask(row.taskId, "worker", e.target.value)
                            }
                          >
                            <option value="">Select</option>
                            {workers.map((w, i) => (
