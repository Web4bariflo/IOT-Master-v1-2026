...
const AddCluster = () => {
  ...
  const [clusters, setClusters] = useState([]); // State to hold clusters
  ...
  const [showAddDevice, setShowAddDevice] = useState(false); // State to show AddDevice
  const [showAllCluster, setShowAllCluster] = useState(false); // State to show all clusters
  ...
  const handleAddCluster = async () => {
    ...
    try {
      ...
      setClusters((prevClusters) => [
        ...prevClusters,
        {
          cluster_id: response.data.message.cluster_id,
          name: response.data.message.cluster_name,
        },
      ]);
    ...
    return (
      <div className="flex flex-wrap gap-4 mb-3">
        {clusters.map((cluster) => (
          <div
            key={cluster.cluster_id} // Ensure to use a unique key for each cluster
            className="flex flex-col p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 w-[250px]" // Set width for each card
          >
            <div className="p-2 bg-white max-h-max shadow-lg max-w-max rounded-md cursor-pointer">
              ...
              <div className="flex w-full justify-between px-2">
                <p className="text-lg text-orange-900 font-semibold mt-4">
                  {cluster.name} {/* Access cluster name from state */}
                </p>
              </div>
            </div>
          </div>
  ...
  return (
    <div>
      ...
      {showAllCluster && (
        <div className="flex-row w-fit gap-4 mb-3">{renderClusterCards()}</div>
      )}
